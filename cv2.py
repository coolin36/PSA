...
class Album():
    ...
    def print(self):
        print("| {0} | {1} | {2} | {3} | {4} | {5} |".format(self._title, self._artist, self._year, self._genre, self._track_no, self._medium_type))

    class Album_library():
        def __init__(self):
            self._album_library_list = []
        
        def init_data(self):
            self.add_album(Album("Reign In Blood", "Slayer", 1986, Genre.METAL, 10, Medium_type.LP))
        ...
        def find_album(self, search_string):
            for album in self._album_library_list:
                if album._string.lower() in album.title.lower():
                    return album
                if album._string.lower() in album.artist.lower():
                    return album
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == "1":
            ...
            track_no = input("Enter number of tracks: ")
            medium_type = input("Enter medium type: ")
            library.add_album(Album(title, artist, year, genre, track_no, medium_type))
            continue
        elif choice == "3":
            input("Enter album title to remove: ")
            album = library.find_album(string)
            album.print()
            confirm = input("Are you sure you want to remove this album? (y/n)")
            if confirm == "y":
            ...
            search_string = input()
            album = library.find_album(search_string)
            album.print()
            continue
        else:
            break